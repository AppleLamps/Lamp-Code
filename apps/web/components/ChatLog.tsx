...
export default function ChatLog({ projectId, onSessionStatusChange, onProjectStatusUpdate, startRequest, completeRequest }: ChatLogProps) {
  ...
  const shortenPath = (text: string) => {
    ...
    // Pattern to match file paths (handles both Unix and Windows paths)
    // Unix: /path/to/file.ext or Windows: C:\path\to\file.ext
    const unixPathPattern = /\/[^\/\s]+(?:\/[^\/\s]+){3,}\/([^\/\s]+\.[^\/\s]+)/g;
    const windowsPathPattern = /[A-Za-z]:\\[^\\\/\s]+(?:[\\\/][^\\\/\s]+){3,}[\\\/]([^\\\/\s]+\.[^\\\/\s]+)/g;

    let result = text.replace(unixPathPattern, (match, filename) => {
      return `.../${filename}`;
    });

    result = result.replace(windowsPathPattern, (match, filename) => {
      return `...\\${filename}`;
    });
