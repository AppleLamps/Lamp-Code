...
async def start_preview(
    ...
    if status == "running":
        # Get existing port and process information
        port = get_preview_port(project_id)
        process_id = get_preview_process_id(project_id)
...
async def get_preview_status(project_id: str, db: Session = Depends(get_db)):
    ...
    # Get port and process information if running
    port = get_preview_port(project_id) if is_running else None
    process_id = get_preview_process_id(project_id) if is_running else None
    url = f"http://localhost:{port}" if port else None
