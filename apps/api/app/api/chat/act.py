...
from app.core.di_setup import create_cli_manager
...
async def execute_act_task(
    ...
    try:
        ...
        # Initialize CLI manager with dependency injection
...
async def run_act(
    ...
    # Get project repository path with fallback
    project_repo_path = project.repo_path
    if not project_repo_path:
        # Fallback: try to get path from filesystem
        from app.services.project.initializer import get_project_path
        project_repo_path = await get_project_path(project_id)

        if project_repo_path:
            # Update database with found path
            project.repo_path = project_repo_path
            db.commit()
            ui.info(f"Updated project {project_id} with found repo path: {project_repo_path}", "ACT API")
...
async def run_chat(
    ...
    if not project_repo_path:
        ...
        if project_repo_path:
            ...
            project.repo_path = project_repo_path
            db.commit()
            ui.info(f"Updated project {project_id} with found repo path: {project_repo_path}", "CHAT API")
